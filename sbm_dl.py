...
class sbm_dataset():
    def __init__(self,args): # 传入的参数是yaml文件里面的一大堆参数
        ...
        edges = self.load_edges(args.sbm_args) # 建立边，数据格式：[[souce, target, weight, time].[...]...]
    ...
    def load_edges(self,sbm_args, starting_line = 1): 
        file = os.path.join(sbm_args.folder,sbm_args.edges_file) # 拼接成该数据集所在地址
        with open(file) as f:
            lines = f.read().splitlines() # f.read().splitlines()没有\n
        edges = [[float(r) for r in row.split(',')] for row in lines[starting_line:]]
        edges = torch.tensor(edges,dtype = torch.long) # 边的数据格式：[[souce, target, weight, time].[...]...]
        return edges
